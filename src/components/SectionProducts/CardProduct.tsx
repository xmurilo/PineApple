...
const CardProduct: React.FC<CardProductProps> = ({ data, handleAddProduct }) => {
            ...
            <div className='md:w-1/2 flex flex-col justify-center mt-4'>
              <div className='text_product max-w-[424px] px-8'>
                ...
                <ul className='flex items-center justify-center gap-x-12 mt-14'>
                  <button className='default_button_background' onClick={() => onOpenModal(item)}>
                    Saiba mais
                  </button>
                  <li>
                    <button className='text-[#006EDB]' onClick={() => handleAddProduct(item)}>
                      Comprar &gt;
                    </button>
                  </li>
                </ul>
        ...
        <div className='p-5 '>
          ...
          <div className='flex items-center justify-center my-5'>
            {selectedProduct?.colors.map((color, index) => (
              <div key={index}>
                <div className='w-11 h-11 rounded-full' style={{ backgroundColor: color }}></div>
              </div>
            ))}
          </div>
          <h2 className='font-SourceSansPro text-[24px] md:text-[40px] text-center font-semibold'>
